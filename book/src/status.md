...
| A persistent appliance identity | **partial** | minted on a fresh store medium and reloaded on every later boot, reported on the console as the device identifier and the SPKI fingerprint, and given up on a [factory reset](design/updates.md#factory-reset) asked for by writing one sector of that medium. An accepted onboarding package writes the delivered device certificate, the trust anchor and the management endpoint into it, and **both of the last two now reach the domains that need them**: the anchor crosses the key delegation to the domain that will validate a management server, and the endpoint is published into a region of its own that the domain which opens the channel maps read-only **and now dials what it names**. What the record can hold and nothing yet writes: the configuration slots — [detail](developers/status-detail.md#the-appliance-identity) |
