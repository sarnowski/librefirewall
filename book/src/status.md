...
| Capability | Status | Notes |
|---|---|---|
| Outbound management channel | **open** | the persistent mutually-authenticated connection of the [channel framing contract](contracts/channel-framing.md). Nothing dials: the TLS stack exists and proves itself against itself in the cryptography domain (below), but nothing carries it — that domain holds no device and no channel, and the endpoint that can now open a TCP connection, resolve a next hop and carry a request over it has no caller (below) |
