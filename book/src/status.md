...
| A persistent appliance identity | **partial** | minted on a fresh store medium and reloaded on every later boot, reported on the console as the device identifier and the SPKI fingerprint. What the record can hold and nothing yet writes: the delivered device certificate, the trust anchor, the management endpoint and the configuration slots — [detail](developers/status-detail.md#the-appliance-identity) |
