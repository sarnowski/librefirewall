...
| Transport active open | **partial** | the TCP stack now opens connections: `SynSent`, a connect entry point, the simultaneous open, RFC 793's reset-not-a-state-change answer to an acknowledgement of something never sent, and an unanswered dial abandoned in silence at the retransmission limit. A neighbour cache exists beside it — bounded, expiring, learning only replies it asked for, and refusing to re-bind a resolved entry — held by unit, property and fuzz tests. **Missing:** the endpoint neither sends an ARP request nor feeds a reply into that cache, there is no gateway in the configuration schema and no route decision, and nothing dials — so no connection has left this appliance and no entry has been learned on a running node |
