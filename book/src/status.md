...
| Appliance identity | **partial** | **The appliance now has an identity that survives a reboot.** A store protection domain owns a block device of its own, and on a fresh medium it mints what the [certificate profile](contracts/certificate-profile.md) defines — 128 bits of device identifier from its own `RDRAND`-seeded generator, an ECDSA P-256 keypair, a self-signed onboarding certificate binding the two — writes all of it behind a device barrier, and reports the identifier and the SPKI fingerprint on the console. On every later boot it loads the same record, holds the three halves of the identity to each other, and reports that the same appliance returned. **Factory reset now takes that identity away**: a request written to one sector of the medium — the only path into a node with no shell, and one that needs the medium in hand — is cleared and made durable before every sector the layout claims is overwritten, and the appliance mints afresh and comes back unowned, having said on the console what it destroyed. Three QEMU scenarios share one medium: the second is held to the first's answer, which is the only shape a persistence claim has, and the third resets it and must come back a different, unowned appliance with the previous scalar occurring nowhere on the medium. **Missing:** the CSR is not served, the device certificate and trust anchor an owner delivers have a place in the record and nothing delivers them, the configuration slot array is written by nothing, the recorder's medium has no reset of its own, and the cryptography domain still generates its own key rather than delegating a signature to this domain — the [signing interface](design/management.md) is shaped for it and its consumer does not exist |
