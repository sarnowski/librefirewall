...
| Transport active open | **partial** | the TCP stack now opens connections: `SynSent`, a connect entry point, the simultaneous open, RFC 793's reset-not-a-state-change answer to an acknowledgement of something never sent, and an unanswered dial abandoned in silence at the retransmission limit — all proved by unit, property and fuzz tests. **Missing:** nothing on the endpoint sends an ARP request or holds an ARP cache, there is no gateway in the configuration schema and no route decision, and nothing dials — so no connection has left this appliance |
