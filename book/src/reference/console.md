...
- `recording-start=<n> recording=resumed recording-generation=<n> recording-sequence=<n>
  recording-opened=<n>` — a recording this boot **continued** rather than started over.
  `recording-start=` is the extent's first sector, the same number the `start=` record above it
  carries, so the two are paired by their value and not by their order. `recording-generation=` and
  `recording-sequence=` are what the **medium** said: the superblock's monotonic generation and the
  segment its writer was in, both decimal, so an operator holding the disk can compare them against
  it number for number. `recording-opened=` is the segment this boot went on to write, which is
  always the one after the segment it read — the previous boot's last segment was never closed, so
  nothing may be appended to it.

  There is no byte offset here on purpose: a resumed recording refuses to serve anything before the
  segment it opened, so an offset into the segment before it is a position nothing can act on.
- `recording-start=<n> recording=fresh recording-rebound=<true|false>` — a recording this boot
  **started over**, and which of the two reasons it did. `false` is an extent whose superblock does
  ...
  `true` is an extent whose superblock decoded and described **a different ring** — the extent was
  rebound, or this is not the device it was — and the appliance recorded over it rather than not
  recording at all. That case carries a `ready` record with `cause=recording-extent-rebound` beside
  it, because what it overwrote was somebody's evidence.
...
One token in this domain is not a refusal to start at all: `recording-extent-rebound` arrives on a
`ready` record, the node goes on recording, and what it says is that the extent held a superblock
describing some other ring and this appliance wrote over it. Not recording at all is the worse
failure for an appliance whose recordings are its evidence — but overwriting somebody's is loud by
design, and this is where it is said.
