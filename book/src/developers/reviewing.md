...
4. **Preconditions.** For each documented precondition delegated to a caller: follow the delegation
   ...
   critical.
5. **Capabilities.** Did `datad/systems/` change? That is a security change requiring human
   sign-off;
...
10. **Observation.** Did the author [observe the change on a running node](engineering.md#verifying-on-a-running-appliance),
    or only test it? For a dataplane change, ask for the capture — `datad/build/image/` holds the
    last run's recordings and serial output, so this costs a `tcpdump -r`, not a re-run.
11. **Verdict.** Report every finding as one line:
    ...
    Example: `CRITICAL datad/crates/virtio/src/queue.rs:212 — used-ring index from the device feeds a
