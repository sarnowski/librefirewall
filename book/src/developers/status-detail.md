...
**What the gate proves.** Every scenario whose management port is reachable — 19 of the 36 system
...
boundary, and that payloads arrive in order under those rewritten headers. Two of the 36 system scenarios
...
- **A boot now commits again after a reboot that restored a version, and none confirms.** Three
...
- **No boot confirms, and the reason is a third defect rather than a gap in the gate.** An appliance
  that ends the session itself — which is what a commit does, and the whole of how the
  fresh-connection rule is enforced — **does not dial again**. Every boot that commits shows one
  `dial-outcome=established` and no second attempt, on a serial that then goes quiet for the rest of
  the run while the recording ring stalls behind a channel that is no longer shipping. A
  confirmation is admissible only over a connection opened after the commit, so until that is fixed
  no boot can reach one however it is driven: the scenario that would have confirmed was written,
  run, and found to be blocked there. What that leaves unobserved on a booted node is the
  confirmation itself, the reversal on an expired deadline, and a commit the medium refused; all
  three are held by host-level cover and by the console vocabulary instead. **This is the largest
  gap in the configuration transaction today**, because commit-confirm exists precisely to bound an
  appliance whose management plane has become unreachable, and a commit that can never be confirmed
  is one that always reverts.
...
Every end-to-end scenario now boots the **release** image, and two of the 36 system scenarios
...
Six of the 36 system scenarios additionally hold the console's own record to the frames and the bytes
...
one of 30 console cause tokens. Two of the 36 system scenarios assert that record
...
with the domain that has a metric to count into, and the 10 scenarios judge the channel the appliance
...
provokes one — the 10 scenarios judge the channel the appliance dials against a real
