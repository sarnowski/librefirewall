...
- **Two readers, and neither holds a durable cursor.** The superblock carries four reader-cursor
  ...
  it and nowhere else, so a reboot starts it at the beginning of the ring and re-ships everything
  the medium still holds. That unregistered cursor is also why no series says how much history
  a recording still holds: a wrap count states that a segment was evicted, and there is no cursor
  for it to have been evicted past. What a cursor the recording has outrun no longer does is stop:
  a resumed recording begins at the segment its boot opened and refuses position zero, so on every
  appliance that has restarted once the very first thing that cursor asks for is refused — and it
  used to be given up on there, which meant a deployed node shipped nothing for the rest of the
  boot. The recorder now answers where each recording begins on every reply and the reader carries
  on from it, saying once per recording what it skipped.
