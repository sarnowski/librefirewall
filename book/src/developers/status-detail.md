...
`tracking="related"` rule is then pushed down the channel the appliance dials, and the same error on
the same flow crosses — attributed to that rule in `librefirewall_rule_hits_total{rule="probe-related"}`, with
...
A resumed recording continues **in** the segment the superblock names, at the offset it names, and
opens a pcapng section header there. The previous boot's segment was never sealed, so its tail past
that offset is unpadded and still holds the wrap before it — and what puts those bytes out of reach
is the resume point itself rather than a floor: placing the write cursor at the durable cursor makes
that segment this boot's *open* one, and a locate reports an open segment readable only as far as
the cursor. Every segment behind it was padded to its full length by the roll that closed it, so the
property holds across any number of restarts. What it buys is that nothing the medium already held
stops being servable, which is what a position a management server acknowledged before the reboot
depends on. A stored write position that is not a whole number of sectors is not a position any run
of this ring leaves: it is refused by name, and the extent is recorded over under a token of its own
rather than continued short.

Each recording says on the console which of the three happened — resumed with the generation, the
segment and the offset the medium held, or fresh with a flag separating an unwritten extent from one
this boot could not continue — because a node with no shell has no other way to tell an operator, and
...
- **Two readers, and one of them holds a durable cursor.** The superblock carries four reader-cursor
  ...
  can serve. The predecessor's unpadded tail is not refused by a floor; it lies beyond this boot's
  write cursor, which is where the ring stops reporting an open segment readable, and every segment
...
boundary, and that payloads arrive in order under those rewritten headers. Two of the 37 system scenarios
assert the console transcript, and one of those boots an image built from a second
document that shares no address and no MAC with the first.

**Submitting a document.** A document reaches this appliance over the authenticated management
channel and over nothing else. The path is four steps and the trust runs one way through it.

The **management** domain owns the port and terminates the TCP connection the channel is dialled
over, and that is the whole of its part: what crosses it is ciphertext, handed to the domain that
holds the keys over the relay those two already shared. **It never sees a document.** It holds two
frame pipelines, so it is the domain an attacker reaches first and the last that should be reading
an attacker's XML — and after this landing it cannot, mapping no part of a submission channel at all.

The **cryptography** domain terminates the TLS session, decodes the frame the document arrives in,
and copies the bytes into a shared region the deciding domain reads. It parses no XML either: what
it holds is a length and a byte range, and the operation word beside them.
...
refusal `rejected=` and `offset=`. A refusal changes nothing; a commit publishes the new image to the
forwarding domain, which switches tables at its next poll boundary.

The datastore **outlives `init`**, which is the whole of what made a second generation possible;
...
Two regions and one channel carry it, `chan_cfg_request` and `chan_cfg_reply`, mirrored so each
domain writes only the direction it speaks in. The requesting domain may not write the reply, because
a domain that could would be able to tell itself a document had been accepted that the appliance is
not running. The channel is granted in **one** direction only: the deciding domain has no polling
loop, so a document written into the region is invisible to it until it is woken, while the asking
domain reads for its answer in a bounded spin — a configuration operation happens inside a session's
own pass and has no continuation a notification could resume.

**There is deliberately no way to read the running document out of a node.** The bytes are not kept
by the domain that decides, and a change is an edit of the version the management server already
holds in its own history rather than a read-modify-write against the appliance. It still follows
that the appliance must be able to *state* every configuration it accepts, which is a semantic rule
the store's version history rests on: a document whose canonical form would outgrow the document
bound is refused with `rendering-too-large` rather than committed.

**The transaction is the four steps that protocol takes apart.** The domain that terminates the
channel decodes a pushed document, hands it to the domain that owns the datastore, and answers with
`UP_CONFIG_VALIDATE_RESULT`: one line of the configuration records' own field vocabulary, composed by
the one function in the tree that composes such a line, so the console and the channel cannot come to
say two different things about one event. A commit names the generation the staging reported and is
refused where the two disagree: the number is asked of the store rather than remembered in the asking
domain. The commit is **provisional** — what it displaced is kept — and the session ends on it, which
is the fresh-connection rule made structural. The deadline lives with the domain that holds the
...
sides of the statable bound), `datad/pds/crypto` (the stepped transaction driven from both ends, every
answer shape, and the deadline that reverts an unconfirmed commit), `datad/crates/pd-runtime`
(the result line's whole grammar, every outcome it can name, and a reason word no vocabulary holds
rendered as a token an operator can look up rather than as a number they cannot), the
`config_transaction` fuzz target over the order of the operations a compromised server chooses, and
the `configuration-submission` QEMU
scenario, which is the evidence that matters: it boots the release image, injects traffic the shipped
policy forwards and traffic it drops, refuses two documents over the channel the appliance dialled
(below), pushes a document that exchanges those two verdicts down the same channel and commits it,
waits for the forwarding domain to report the committed generation, and injects again — both verdicts
reversed, with the totals rising across the swap rather than resetting. The push is a real transaction
and not a single call: each document is staged on its own, the appliance answers each with a result
frame naming an outcome and a generation, and only the last of them is committed.
...
*A refused document moves nothing.* The `configuration-submission` scenario stages a malformed
document — unterminated, so the *reader* stops it — and the appliance answers a result frame carrying
`generation=1 outcome=refused rejected=malformed offset=27`, the generation still reading the one the
image booted.
...
scenario stages one: the shipped document with its two rules given one id. It is answered
`generation=1 outcome=refused rejected=duplicate-identifier offset=0`, a reason about an *object*
rather than a byte, and three things then hold together. The generation the result names is the one
still running; the document staged after both refusals is staged at the **very next** generation, so
neither refusal consumed one or left a candidate behind for the commit to pick up; and the traffic
injected after that commit gets the pushed policy's verdicts and not a mixture, which is the
dataplane's own statement that what reached its table was one whole document. The refusal is compared against
...
- **Rollback reaches one version back and no further, though the history it would walk now exists.**
  The datastore keeps what a provisional commit displaced, so the change a commit-confirm cycle
  failed to confirm is undone — but that is one step, not the [configuration
  design](../design/configuration.md)'s return to an *earlier version*. What has changed is the
  other half: the store medium's fixed configuration slot array **is** written now, so a committed
  version outlives a reboot and a later boot comes up on it. The domain that terminates the channel
  keeps the staged bytes past the commit — the deciding domain deliberately does not keep them — and
  hands them to the domain that owns the medium once the deciding domain has assigned the
  generation; that domain snapshots the region, ranges the claimed length against it and against the
  document bound, refuses a generation that does not advance past the array, and writes the slot
  **before** the record naming it so a power cut costs the commit rather than the record. Every boot
  reads the running slot back and holds it to the digest the record carries for it. What is still
  missing is the walk backwards: nothing reads an *older* slot, so a rollback reaches the
  configuration that was running a moment ago and no earlier version, and the array is a history
  that can be read by an operator on the console and not yet by the appliance itself. What is
...
- **A boot now commits again after a reboot that restored a version.** Three
  scenarios push the whole staging-and-commit transaction down a session the appliance greeted
  first; a pair proves a committed version is durable across a reboot; and a boot after that pair
  reads their medium back and is reconfigured **again**, which is the sequence that catches a
  version numbered onto one the medium already holds. It is judged on both of the store domain's
  records — the version it restored, and the new slot it wrote — and on the generation its commit is
  *answered* with, which is the number the defect moved.
- **And a boot now confirms, over the second connection the appliance opens for itself.** It commits
  a document into a slot of its own medium provisionally, is left alone while the session that
  committed it ends, and keeps that version over the connection the appliance dials afterwards —
  which is the only place a confirmation is admissible. Three independent facts settle it: the
  result line the appliance answers the confirmation with, the server's own second greeting from the
  appliance, and the appliance's own record of a second attempt reported established. A boot with
  the first two and not the third would be asserting the protocol with no transport underneath it.
- **What made that boot look impossible was the wait, and it is worth recording because a defect was
  escalated on it.** The reconnection is not free, and nothing in the harness had accounted for what
  it costs. A commit closes the connection from the appliance's own end, so the endpoint above the
  transport holds the session until the transport gives that connection's slot back — a minute — and
  only then does the redial schedule draw its next wait, below a bound the agreed greeting had just
  reset to one second. So the appliance owes its second connection a minute and a second or two, and
  the first attempt at this boot gave the second session exactly the passes the first one gets:
  sixty seconds of them. It therefore stopped looking at the very instant the appliance's transport
  was giving the slot back, a second and a half before the second connection came up, and the serial
  it left behind — one `dial-outcome=established`, then a minute of quiet while the log ring stalled
  behind a channel that was no longer shipping — was read as an appliance that does not dial again.
  It does, twice over on a booted image: sixty seconds to the slot, a wait drawn below one second,
  and a second session established about sixty-two seconds after the first one ended. Neither the
...
- **The schedule does not start until the connection is gone, and that costs a minute on a session
  this appliance ended itself.** The endpoint holds a finished session until the transport gives its
  connection's slot back, which is what keeps the ending it reports answerable for as long as
  anybody asks — and a connection closed from this end sits in `TIME_WAIT` for a minute first. So a
  channel the peer reset re-dials within a second of the reset, and a channel this appliance closed —
  which is what every commit does, the fresh-connection rule being enforced by closing — re-dials
  about a minute later. It is correct rather than fast, and it is worth stating because it is not
  derivable from the backoff above: read as a schedule alone the appliance looks a second away from
  its next attempt when it is a minute away, and a boot that gave it a second read an appliance that
  never dialled again.
...
else ever moved it, so leaving it declared would have published a counter that could only ever read
zero — a series an operator would reasonably take as evidence that nobody had read the policy.

**Two families are declared and carried by nothing**, which is the honest cost of removing the
metrics surface. `librefirewall_interface_info` — the info gauge whose labels joined a counter series to the
...
- **No surface of their own**, so of the debug dump only the two recordings can be asked for, and
  the readings and the retained records travel framed inside them
  ([detail](#full-port-role-model)).
...
with the domain that has a metric to count into, and the 11 scenarios judge the channel the appliance
...
that holds the identity publishes before the forwarding domain reads, on every one of the 36 boots.
...
provokes one — the 11 scenarios judge the channel the appliance dials against a real
...
- **A decoded record becomes a row in ClickHouse.** The ingest seam's deployed implementation holds
  ...
  ```
- **The metric surface becomes rows too, and `metric_samples` now has a producer.** The appliance
  publishes its whole metric surface into the log recording about once a second as a PEN-tagged
  Custom Block, so the numbers ride the same verbatim ring bytes as everything else and need no
  second transport. This server tells such a block from the padding that shares its type and
  enterprise number by the first byte of its data — empty data, or a leading zero, is padding —
  decodes it against a catalogue generated from the appliance's own `lfw_metrics`, and writes one
  row per slot. The catalogue is generated rather than kept by hand: `cargo run -p xtask --
  metric-catalogue` writes `ctrld/priv/metric_catalogue.json` and the appliance's fast gate refuses
  a tree where the committed file and the code have parted, so a four-hundred-entry table does not
  exist twice in two languages. Every snapshot carries a fingerprint over that table, compared
  before a single slot is mapped, and a snapshot from another catalogue yields **no rows at all**
  rather than numbers under the wrong names — a wrong measurement being the one failure nothing
  downstream can question. What an operator reads it back with is a query naming one appliance, one
  family and a window:

  ```sql
  SELECT observed_at, labels['domain'] AS domain, value
  FROM metric_samples
  WHERE device_id = '<the appliance>' AND family = 'librefirewall_forwarded_frames_total'
  ORDER BY observed_at
  ```

  Two things a reading deliberately does not carry. The per-interface information and the per-rule
  hit counts are absent: which of those exist comes from the committed configuration rather than
  from the catalogue, so neither has a fixed slot and neither could be covered by a fingerprint that
  changes with a policy commit. And a counter above 2^53 is **refused by name** rather than stored,
  `metric_samples.value` being a `Float64` whose integers are exact only that far — a rounded
...
- **The numbers cross a page rather than a widened grant.** The recorder is the only domain that
  ...
  read-write and recorder read-only — carries a whole reading between them, and the eleven-way
  read grant that would otherwise have gone to the domain holding a block device was not made. The
