...
| The system description agrees with the constants the domains compile against: every region's extent, cacheability and per-grant permissions, the **exact** set of domains that map it, both I/O-port windows against the constants the drivers form addresses from, the one interrupt grant against the input the domain programs its timer to drive and the delivery mode its handler is written for, every channel end's notify direction, that no domain reaches an interrupt and a peer's channel through one id, the port→driver attribution, and that each of the 156 mappings is named by a `setvar_vaddr` | `xtask test` (`sysdesc::check`) |
