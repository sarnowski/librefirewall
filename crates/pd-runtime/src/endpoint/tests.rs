...
impl Fixture {
    ...
    fn unaddressed() -> Self {
        ...
        Self {
            ...
            handover,
            receive,
            stage: EndpointStage::attach(EndpointRegions {
                receive,
                receive_returns,
                receive_pool,
                transmit,
...
fn a_fresh_stage_has_seen_nothing_and_an_empty_pipeline_leaves_it_that_way() {
    ...
    assert_eq!(unaddressed.stage.poll(), 0);
    assert_eq!(
        unaddressed.stage.counters(),
        EndpointStageCounters::default()
    );
...
fn a_frame_this_port_does_not_answer_leaves_the_transmit_pipeline_untouched() {
    ...
    assert_eq!(fixture.stage.poll(), 2);
...
fn a_full_return_ring_stops_the_drain_and_is_counted() {
    ...
    assert_eq!(fixture.stage.poll(), published as usize);
    ...
    assert_eq!(fixture.stage.poll(), 0, "and now the ingress ring is empty");
}
...
proptest! {
    ...
    fn every_pass_is_bounded_and_counts_only_believable_lengths(
        ...
        for (head, tail) in forged {
            ...
            prop_assert!(fixture.stage.poll() <= DRAIN_LIMIT);
        }
    }
}
