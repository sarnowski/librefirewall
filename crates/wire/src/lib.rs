...
/// A reference to one pool buffer moving through a queue.
///
/// `buffer` indexes the shared buffer pool and `len` is the number of valid
/// bytes the producer wrote. Holding a descriptor is what "owning" the buffer
...
impl Descriptor {
    ...
    pub const ZERO: Self = Self { buffer: 0, len: 0 };

    #[must_use]
    pub const fn new(buffer: u32, len: u32) -> Self {
        Self { buffer, len }
    }
}
...
const _: () = assert!(size_of::<Descriptor>() == 8);
...
mod tests {
    ...
    fn zero_is_default() {
        ...
        assert_eq!(Descriptor::ZERO, Descriptor::new(0, 0));
    }

    #[test]
    fn fields_round_trip() {
        let d = Descriptor::new(7, 42);
