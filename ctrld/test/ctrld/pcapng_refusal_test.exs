defmodule Ctrld.PcapngRefusalTest do
  ...
  # A refusal is read by whoever is holding the broken recording, so the sentence
  # has to be one: prose, ending without a full stop so a caller can place it in
  # one of its own, and never the raw term, which would put an Elixir tuple in
  # front of an operator. `describe/1` is typed as a string, so asserting that it
  # is one asserts nothing; what is worth holding is that it says something and
  # says it in words.
  defp described(reason) do
    description = Pcapng.describe(reason)
    refute description == ""
    refute String.ends_with?(description, ".")
    refute description == inspect(reason)
    assert String.match?(description, ~r/^[a-z0-9]/)
    description
  end
