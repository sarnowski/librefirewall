This is a web application written using the Phoenix web framework. It is the `ctrld` component of
librefirewall; the repository-root `AGENTS.md` is the working agreement and governs where the two
disagree. There is no Mix toolchain on the host: every command runs inside the pinned builder —
`make ctrld-test` is the gate, `make ctrld-server` the development server (both from the
repository root).
